...
for St_loop = 1:size(St,1)
    ...
    for kx_loop = 1:size(kx,1)
        ...
        [r,su,ss,sv,U0,dU0,dr]  = resolventSVD(Re,k,m,om,N,rc2,rmax,nsvd);
...
figure;
h1 = semilogy(St, squeeze(SS(1,:,1)), 'ko');
hold on;
h2 = semilogy(St, squeeze(SS(2,:,1)), 'ro');
h3 = semilogy(St, squeeze(SS(3,:,1)), 'bo');
...
figure;
hold on;
h1 = plot(r, abs(squeeze(SV(1:N, 1, 6))), 'k-', 'linewidth', 2);

figure;
hold on;
h1 = plot(r, abs(squeeze(SV(N+1:2*N, 1, 6))), 'r-', 'linewidth', 2); %#ok<*NASGU>

figure;
hold on;
h1 = plot(r, abs(squeeze(SV(2*N+1:3*N, 1, 6))), 'b-', 'linewidth', 2);
